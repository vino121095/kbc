    ...
    const fetchBusinessProfile = async () => {
      ...
      try {
        ...
        if (data.success && data.data) {
          ...
          setSnackbar({
            open: true,
            message: data.message || t('errorFetchingBusinessProfile'),
            severity: 'error'
          });
        }
      } catch (error) {
        ...
        setSnackbar({
          open: true,
          message: t('errorFetchingBusinessProfile'),
          severity: 'error'
        });
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!businessProfile) {
        setSnackbar({
          open: true,
          message: 'Business profile data not loaded',
          severity: 'error'
        });
        return;
      }
  ...
  return (
    <Box>
      ...
      <Box p={2}>
        ...
        {/* Tags */}
        <Box sx={{ mt: 2 }}>
          <Typography fontWeight="bold" mb={1}>{t('tags')}</Typography>
